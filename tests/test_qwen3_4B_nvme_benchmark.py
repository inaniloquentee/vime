...
def benchmark_args(phase, root, num_rollouts, *, checkpoint_interval=20, resume_step=None, deterministic=False):
    ...
    if phase in ("capture", "gpu", "gpu-repeat"):
        for name in (
            "--stream-optimizer-state-to-disk", "--offload-train-disk-dir",
            "--offload-train-disk-chunk-mb", "--stream-optimizer-state-moment-dtype",
        ):
            set_option(tokens, name)
    if phase == "capture":
        ...
        set_option(tokens, "--custom-megatron-before-train-step-hook-path",
                   "test_qwen3_4B_nvme_benchmark.measure_step")
...
def measure_step(args, rollout_id, step_id, model, optimizer, scheduler):
    ...
    context = {
        "rollout": rollout_id, "step_in_rollout": step_id, "step": rollout_id * 2 + step_id,
        "rank": dist.get_rank(),
    }
        ...
        def measured_step():
            ...
            if current["step"] in (0, 1, 43):
                record["model_samples"] = [
                    ...
                    for chunk in model for param in chunk.parameters()
                ]
            if current["step"] == 43:
                import hashlib
...
def prepare_replay(root, num_rollouts):
    ...
    for index in range(num_rollouts):
        ...
        manifest.append({"rollout": index, "source": str(source),
...
if __name__ == "__main__":
    ...
    parser.add_argument("phase", choices=["capture", "prepare-replay", "gpu", "gpu-repeat", "nvme", "compare", "soak", "resume"])
    ...
    parser.add_argument("--deterministic", action="store_true", help="Use deterministic Megatron/TE/NCCL controls for replay")
    ...
    if options.phase == "prepare-replay":
        ...
        train_args = benchmark_args(
            options.phase, options.root, options.num_rollouts,
            checkpoint_interval=options.checkpoint_interval, resume_step=options.resume_step,
            deterministic=options.deterministic,
        )
        (run / "command.txt").write_text(train_args + "\n")
        e2e.U.execute_train(
            train_args=train_args, num_gpus_per_node=8, megatron_model_type=e2e.MODEL_TYPE,
            extra_env_vars={
