...
class NVMeOptimizerStateStore:
    ...
    @torch.no_grad()
    def restore_main_to_model_params(self) -> None:
        """Restore checkpointed FP32 mains into the live model buffer without
        narrowing them through BF16 model parameters first."""
        for bucket in self.buckets:
            bucket._move(("main",), to_disk=False)
            self._copy_main_to_model_params(bucket.entries)
            ...
            "buckets": [
                {
                    ...
                    "state_steps": [
                        int(state["step"].item() if torch.is_tensor(state.get("step", 0)) else state.get("step", 0))
                        for entry in bucket.entries
                        for state in [bucket.adam.state.get(entry.main_param, {})]
                    ],
...
def setup_optimizer_state_streaming(args, optimizer) -> None:
    ...
    for dist_opt in getattr(optimizer, "chained_optimizers", [optimizer]):
        assert isinstance(
        ...
        store = NVMeOptimizerStateStore(
            ...
            allow_fresh_state=getattr(args, "no_load_optim", False),
        )
